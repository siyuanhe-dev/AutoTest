...
static HRESULT TryRelaunchElevated()
{
    ...
    if ((INT_PTR)result > 32) {
        return S_OK; // launched elevated process
    }
    // User probably cancelled or failed
    return HRESULT_FROM_WIN32(ERROR_CANCELLED);
}

extern "C" __declspec(dllexport) HRESULT CheckAndFixDependencies()
{
    bool hasWpr = false, hasWpa = false;
    std::wstring path;
    ...
    // If not elevated, try to relaunch elevated. This will prompt UAC.
    if (!elevated) {
        HRESULT hr = TryRelaunchElevated();
        if (SUCCEEDED(hr)) {
            // We launched an elevated instance; caller should exit and let elevated instance continue
            return S_FALSE;
        }
        // If relaunch failed or user cancelled, fall through to error handling
    }

    // If missing WPR/WPA, show an error message and return failure
    if (!hasWpr || !hasWpa) {
        std::wstring msg = L"Required dependencies not found:\n";
        if (!hasWpr) msg += L" - wpr.exe (Windows Performance Recorder)\n";
        if (!hasWpa) msg += L" - wpa.exe (Windows Performance Analyzer)\n\n";
    ...
    // If we reach here, elevation was required but relaunch failed
